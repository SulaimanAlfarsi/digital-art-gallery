        ...
        >
          {[right, left].map((name, rail) =>
            Array.from({ length: cards }, (_, i) => {
              // Offset the left rail by a full set of cards so each side shows
              // a different run of images instead of mirroring the right.
              const len = Math.max(images.length, 1);
              const img = images[(i + rail * cards) % len];
              return (
                <div
                  key={`${name}-${i}`}
